/**
 * Controller deep-link / session param helpers (no Firebase side effects).
 */
 ...
 */
export function resolveControllerParam(key, { searchParams, session } = {}) {
  ...
  if (key === "event") {
    return session?.eventId || sessionStorage.getItem("selectedEvent") || "";
  }
  if (key === "court") {
    return session?.courtId || sessionStorage.getItem("selectedCourt") || "";
  }
