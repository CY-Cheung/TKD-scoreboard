...
產品 Landing Hero 頁：[`/welcome`](https://cy-cheung.github.io/TKD-scoreboard/welcome)（路由 `src/Pages/Landing/`）。
...
**Session**：Court Setup 後 `{ eventId, courtId }` 存入 `sessionStorage`；`ProtectedRoute` 守護需登入場地嘅路由；未有 session 會導去 `/welcome`。
...
| `src/Pages/Landing/` | 產品 Landing Hero |
...
| Landing Hero | `Pages/Landing/` |

---

## 安全 · 未入盒 · 延伸

**Court-level Locking**：坐正 J1–J3 嘅 `deviceId` 先改分；Admin 靠 Google auth；Event 靠建立者。詳見 `database.rules.json`。

**計劃中**：Persistent Token 重連、hostStatus 離線警示（詳見 [`TODO_WT2026.md`](TODO_WT2026.md)）

**已實作（WT 2026）**：Technical Card、IVR — 見 [`TODO_WT2026.md`](TODO_WT2026.md)
